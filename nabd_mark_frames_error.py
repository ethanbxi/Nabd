...
def render_one(t, px=30, art=None, bg=None):
    """The mark posed at `t` on the error timeline, as a PIL image.

    The single definition of this raster path, as in nabd_mark_frames: render()
    bakes the flipbook with it and render_readme_media.py composites the
    failure GIF with it, so the points-vs-pixels correction exists once.
...
def render(outdir, scales=(1.0,), px=30, art=None, bg=None, suffix=""):
    ...
    `suffix` puts the frames in "1x-fail" beside the card set they belong with;
    the banner resolves one directory per scale and field, so a second tree
    would be a second path to keep in step.
    """
