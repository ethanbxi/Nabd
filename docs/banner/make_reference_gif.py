# -*- coding: utf-8 -*-
"""Render the save banner to a GIF, for looking at rather than for shipping.

    python make_reference_gif.py reference

Two files land in the output directory:

  banner-4120ms.gif   the whole banner on the shell backdrop, actual timeline
  mark-closeup.gif    the mark alone, large, so the horn roots and the turn
...
# ── colour ───────────────────────────────────────────────────────────────
PURPLE = (0x6C, 0x3B, 0xAA)
PURPLE_LIGHT = (0x9B, 0x6B, 0xD8)
...
def _font(mono, px):
    ...
    name = "DejaVuSansMono.ttf" if mono else "DejaVuSans-Bold.ttf"
    try:
    ...
    while t < B.TOTAL_MS:
        ...
        if w > 0 and h > 0 and f.alpha > 0.002:
            ...
            if f.copy > 0.002:
                ...
                y = (inner_h - block) / 2
                d.text((x, y), "Nabbed", font=title_f, fill=mix(CREAM))
                d.text((x, y + s(TITLE_PX) * 1.2), "5:00 - 1.2 GB",
...
def main():
    ...
    a = save_gif(banner_frames(), out / "banner-4120ms.gif")
    b = save_gif(mark_frames(), out / "mark-closeup.gif")
    from PIL import Image
    for p in (a, b):
        im = Image.open(p)
        total, n = 0, 0
