...
def pose_svg(f, size=None, art=CREAM, box=ANIM, ring_stroke_path=None,
    ...
    eo = 0.25 + 0.75 * f.eyes
    # `wink` shuts the right eye only; `shut` shuts both. The save banner's
    # Frame has no `shut` field at all, so getattr keeps its output identical
    # -- byte-for-byte, which the error package's checker asserts.
    sh = getattr(f, "shut", 0.0)
    w = 1 - max(f.wink, sh) * 0.94
    w_l = 1 - sh * 0.94
    ...
    def eye(side, d, winks):
        # `winks` is fixed per side, not derived from `near`: the wink belongs
        ...
        t = "translate(%.3f,0) %s" % (dx, _about(
            ...
            "scale(%.5f,%.5f)" % (k, eo * (w if winks else w_l))))
