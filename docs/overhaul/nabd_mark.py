...
def pose_svg(f, size=None, art=CREAM, box=ANIM, ring_stroke_path=None):
    ...
    def horn(side, d):
        ...
        return '<g transform="%s"><path d="%s"/></g>' % (
            _about(HORN_ROOT[side][0], HORN_ROOT[side][1],
                   "scale(%.5f,%.5f)" % (k * grow, grow)), d)
    ...
    def eye(side, d, shut):
        ...
        return '<g transform="%s" opacity="%.4f"><path d="%s"/></g>' % (t, f.eyes, d)
    ...
    return (
        ...
        'stroke-dasharray="%.2f" stroke-dashoffset="%.2f"/></g>'
        '<g transform="%s" opacity="%.4f">%s%s</g>'
        '%s%s</g></svg>'
        % (box[0], box[1], box[2], box[3], dim, head, art,
           ring_t, ring_stroke_path, art, SW, RING_LEN, RING_LEN * (1 - f.ring),
           horn_t, min(1.0, f.horns * 2.2), horn("l", HORN_L), horn("r", HORN_R),
           eye("l", EYE_L, near == "l"), eye("r", EYE_R, near == "r")))


def ring_arc(r=None):
    """The ring as one stroked arc, so it can be drawn with a dash offset.

    Starts at the gap's trailing edge and takes the LONG way round in the
    negative direction, which is the counter-clockwise rewind the banner has
    always done. large-arc with the other sweep flag traces the 65 deg bite.
    """
    from math import cos, sin, radians
    r = R if r is None else r
    p = lambda a: (CX + r * cos(radians(a)), CY + r * sin(radians(a)))
    (x0, y0), (x1, y1) = p(A_START), p(A_END)
    return "M %.2f,%.2f A %.2f,%.2f 0 1 0 %.2f,%.2f" % (x0, y0, r, r, x1, y1)
