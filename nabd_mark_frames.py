...
def render_one(t, px=30, art=None, bg=None):
    """The mark posed at `t`, as a PIL image.

    The single definition of the raster path: render() bakes the flipbook with
    it, and render_readme_media.py composites its GIF with it, so the
    points-vs-pixels correction below exists once rather than twice.
    ...
    # transparency, so the horn and eye fades are mixed into the fill against
    # this exact colour. See nabd_mark._mix.
    svg = M.pose_svg(B.sample(t), size=size, art=art or M.CREAM, bg=bg_hex)
    drawing = svg2rlg(BytesIO(svg.encode("utf-8")))
    # svglib reads the SVG's width as POINTS, so a 60 px request comes back
    # 45 px. Rescale the drawing explicitly.
    k = float(size) / float(drawing.width)
    drawing.width = drawing.height = size
    drawing.scale(k, k)
    return renderPM.drawToPIL(drawing, bg=HexColor(bg_hex))


def render(outdir, scales=(1.0,), px=30, art=None, bg=None, suffix=""):
    """Bake the flipbook to PNGs, one directory per scale.

    `suffix` is appended to the per-scale directory name, so the failure
    field's frames land in "1x-fail" beside the card set they belong with. The
    banner resolves one directory per scale and field; splitting the mark into
    a second tree would mean two paths to keep in step.
    """
    outdir = pathlib.Path(outdir)
    total = 0
    for sc in scales:
        d = outdir / ("%gx%s" % (sc, suffix))
        d.mkdir(parents=True, exist_ok=True)
        size = int(round(px * sc))
        for idx, t in plan():
            render_one(t, px=size, art=art, bg=bg).save(
                str(d / ("mark_%04d.png" % idx)))
            total += 1
    return total
